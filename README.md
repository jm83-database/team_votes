...
- **실시간 투표 마감 시간 표시**: 투표 종료까지 남은 시간을 실시간으로 표시
...
- 선택 진행률 실시간 표시
- 직관적인 팀 선택 인터페이스
- **실시간 투표 마감 타이머**: 투표 종료까지 남은 시간을 시/분/초 단위로 표시
- **시각적 경고 표시**: 빨간색 박스와 펄스 애니메이션으로 마감 시간 강조
- **정확한 시간 입력**: 시간 선택 시 스크롤 방지로 입력 정확도 향상
...
5. **투표 마감 시간 확인**: 화면 상단의 빨간색 박스에서 남은 시간 실시간 확인
6. 필요한 개수의 팀 선택 완료 후 "투표하기" 버튼 클릭
7. **투표 결과 확인**: 투표 직후 5초간 선택한 팀 정보 표시
8. **자동 전환**: 카운트다운 후 자동으로 "투표 완료" 상태로 변경
...
7. "학생 현황" 탭에서 학생별 투표 상태 확인 (다중 투표 시 선택한 모든 팀 표시)
...
- **실시간 마감 타이머**: 투표 종료까지 남은 시간을 실시간으로 표시
- **정확한 시간 입력**: 마우스 휠 스크롤 방지로 시간 설정 정확도 향상

### 시스템 품질
- **지능형 순위 시스템**: 공동 순위 자동 처리 및 올바른 순위 건너뛰기
- **시각적 순위 구분**: 금/은/동 메달 색상으로 등수 구분
- **다중 투표 현황 표시**: 관리자 페이지에서 학생이 선택한 모든 팀 확인 가능
- **보안 강화**: 관리자 기능에 이중 비밀번호 확인 시스템
- **사용자 친화적**: 실시간 가이드와 명확한 상태 안내

### 개발 품질
- **모듈화된 아키텍처**: 상수, API, 훅, 컴포넌트가 체계적으로 분리
- **코드 재사용성**: 공통 로직을 훅과 유틸리티로 추상화
- **중앙화된 상태 관리**: 일관된 패턴으로 상태 관리
- **API 추상화**: 중복 없는 깔끔한 API 호출 구조
- **타입 안전성**: 상수 사용으로 런타임 오류 방지
...
- 시간 형식 (HH:MM) 정확성 확인
- 시간 입력 시 마우스 휠 대신 키보드나 드롭다운 사용

### 투표 마감 시간 표시 오류
- 투표 종료 시간이 올바르게 설정되었는지 확인
- 브라우저 시간대 설정 확인
- 서버와 클라이언트 시간 동기화 확인

### 코드 수정 및 확장 가이드
- **상수 변경**: `CONSTANTS` 객체에서 한 번만 수정
- **새 API 추가**: `api` 객체에 새 메서드 추가
- **UI 컴포넌트**: 기존 재사용 컴포넌트 활용 우선
- **상태 관리**: 커스텀 훅 패턴 따라 구현
- **에러 처리**: `useApi` 훅의 표준 패턴 사용
...
const api = {
    student: {
        ...
        vote: (voteData) => api.post('/api/vote', voteData)
    },
    admin: {
        getVoteConfig: () => api.get('/api/admin/vote-config'),
        resetVotes: () => api.post('/api/admin/reset', {...})
    }
};
```

#### 커스텀 훅 활용
- `useMessage()`: 메시지 표시 및 자동 숨김
- `useApi()`: API 호출 상태 관리 (로딩, 에러)
- `useCountdown()`: 카운트다운 타이머
- `useVoteTimer()`: 투표 마감 시간 실시간 표시
- `useFormData()`: 폼 데이터 상태 관리

#### 재사용 가능한 컴포넌트
- `Modal`: 기본 모달 구조
- `ConfirmModal`: 확인/경고 모달
- `FormModal`: 폼 기반 모달
- `InputField` / `SelectField`: 표준화된 입력 필드
- `TimeSelector`: 커스텀 시간 선택기

### 개선된 개발 경험

#### 코드 중복 제거
- **Before**: 동일한 fetch 로직이 10+ 곳에서 반복
- **After**: `api` 객체로 중앙화된 API 호출

#### 유지보수성 향상
- **상수 변경**: 한 곳에서 수정으로 전체 반영
- **기능 추가**: 기존 패턴 재사용으로 빠른 개발
- **버그 수정**: 영향 범위가 명확히 분리됨

#### 타입 안전성
- 문자열 하드코딩 → 상수 사용으로 오타 방지
- 일관된 네이밍 컨벤션 적용

## 개발 가이드

### 새로운 기능 추가 시
1. **상수 정의**: `CONSTANTS`에 필요한 값들 추가
2. **API 함수**: `api` 객체에 엔드포인트 추가
3. **커스텀 훅**: 필요시 상태 관리 로직 분리
4. **컴포넌트**: 재사용 가능한 UI 컴포넌트 활용

### 코딩 컨벤션
- 상수는 `CONSTANTS` 객체 사용
- API 호출은 `api` 객체 메서드 사용
- 상태 관리는 커스텀 훅 우선 고려
- UI 컴포넌트는 재사용성을 염두에 두고 설계

## 버전 히스토리

### v3.0.0 (최신) - 대규모 리팩토링
- **전체 코드 아키텍처 개선**: 모듈화 및 구조화
- **상수 중앙 관리**: 모든 하드코딩 값들을 CONSTANTS로 분리
- **API 추상화**: 중복된 fetch 로직을 api 객체로 통합
- **커스텀 훅 도입**: useMessage, useApi, useCountdown, useVoteTimer 등
- **재사용 컴포넌트 분리**: Modal, ConfirmModal, FormModal, InputField 등
- **코드 중복 90% 감소**: DRY 원칙 적용으로 유지보수성 대폭 향상
- **확장성 개선**: 새로운 기능 추가가 용이한 구조로 변경
- **개발자 경험 향상**: 일관된 패턴과 명확한 구조

### v2.2.0
- **실시간 투표 마감 타이머 추가**: 학생용 페이지에 남은 시간 실시간 표시
- **시간 입력 스크롤 방지**: 관리자 시간 설정 시 마우스 휠 비활성화
- **다중 투표 현황 표시 개선**: 관리자 페이지에서 3개 팀 모드 투표 현황 정확히 표시
- **사용자 경험 향상**: 시각적 경고 표시 및 펄스 애니메이션 추가
- 도움말 모달 및 문서 업데이트
